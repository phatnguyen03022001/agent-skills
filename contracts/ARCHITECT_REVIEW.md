...
Canonical new reviews serialize exact reviewed-report identity, material compliance, and final judgment. `independence`, `gap_disposition`, `follow_up_tasks`, `promotion_readiness`, and `notes` are included only when materially applicable; they do not duplicate Executor narration. Expanded-v3 review fields remain valid compatibility input and retain their existing validation when present.

...
When Agent Runtime is the selected local transport, prefer `terminal_exec` as the carrier for bounded non-interactive deterministic bundles. Reserve `terminal_start`/`terminal_poll`/`terminal_control` for genuinely interactive or long-running work that cannot be truthfully completed as one bounded one-shot invocation. This is a selection rule within the existing four-tool surface and does not add a bundle API or runtime orchestration capability.

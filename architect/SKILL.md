...
When authoring task PROOF, apply the Task Protocol validity classes and Executor execution-bundle semantics rather than restating them: reuse exact ACCEPTED `IMMUTABLE` candidate correctness evidence only when candidate, review, and governing authority identities are unchanged, the prior evidence actually covers the exact currently required predicate, and the governing/predicate semantics and relevant dependencies for that proof remain applicable. Same candidate identity alone is never sufficient. In a fresh binding, discard context-local resolution assumptions and freshly re-resolve the exact prior canonical evidence identity before relying on it; do not require acceptance-equivalent rereads or reruns solely because the binding or a later promotion/activation consequence is fresh when those applicability predicates remain satisfied. Refresh only evidence invalidated by that next consequence (including current remote refs/topology/ancestry/current capability and fresh post-mutation remote identity), or when explicit stronger assurance, contradiction, or materially mutable/runtime truth requires it. Make deterministic local proof bundle-friendly so one bounded invocation may contain ordered serial phases and safely parallel independent checks while preserving every assurance predicate instead of forcing one model/runtime synchronization boundary per command.

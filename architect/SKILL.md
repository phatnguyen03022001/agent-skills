...
When authoring task PROOF, apply the Task Protocol validity classes and Executor execution-bundle semantics rather than restating them: reuse unchanged exact ACCEPTED `IMMUTABLE` candidate correctness evidence when candidate, review, and governing authority identities are unchanged, and do not require acceptance-equivalent rereads or reruns solely because a later promotion/activation consequence begins. Refresh only evidence invalidated by that next consequence (including current remote refs/topology/ancestry/current capability and fresh post-mutation remote identity), or when explicit stronger assurance, contradiction, or materially mutable/runtime truth requires it. Make deterministic local proof bundle-friendly so one bounded invocation may contain ordered serial phases and safely parallel independent checks while preserving every assurance predicate instead of forcing one model/runtime synchronization boundary per command.
